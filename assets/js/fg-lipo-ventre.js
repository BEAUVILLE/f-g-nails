...
function lang(){try{const x=window.FG_I18N&&FG_I18N.getLang&&FG_I18N.getLang();if(LANGS.includes(x))return x}catch(_){ }const h=(document.documentElement.lang||'fr').slice(0,2).toLowerCase();return LANGS.includes(h)?h:'fr'}
function apply(){const t=TEXT[lang()]||TEXT.fr;document.querySelectorAll('[data-i18n="lipo"]').forEach(el=>{el.textContent=t.name;if(el.tagName==='OPTION')el.value='Lipocavitation ventre'});document.querySelectorAll('[data-i18n="lipo_desc"]').forEach(el=>el.textContent=t.desc);document.querySelectorAll('[data-i18n="main_lead"]').forEach(el=>el.textContent=t.lead);document.querySelectorAll('a[href*="service=Lipocavitation"]').forEach(a=>{const u=new URL(a.getAttribute('href'),location.href);u.searchParams.set('service','Lipocavitation ventre');a.setAttribute('href',u.pathname.replace(location.pathname.substring(0,location.pathname.lastIndexOf('/')+1),'')+u.search+u.hash)});const select=document.getElementById('service');if(select){const requested=new URLSearchParams(location.search).get('service');if(requested&&/^lipocavitation(?:\s+ventre)?$/i.test(requested))select.value='Lipocavitation ventre'}}
document.addEventListener('DOMContentLoaded',apply,{once:true});window.addEventListener('load',apply,{once:true});document.addEventListener('click',e=>{if(e.target.closest('[data-fg-langbar] button'))setTimeout(apply,0)},true);new MutationObserver(apply).observe(document.documentElement,{attributes:true,attributeFilter:['lang','dir']});setTimeout(apply,80);setTimeout(apply,500);
})();
...
(()=>{if(document.querySelector('script[data-fg-lg25]'))return;const s=document.createElement('script');s.src='assets/js/fg-media-lg25.js?v=20260810-v2';s.defer=true;s.dataset.fgLg25='1';document.head.appendChild(s)})();
(()=>{if(document.querySelector('script[data-fg-voice]'))return;const s=document.createElement('script');s.src='assets/js/fg-voice.js?v=20260810-v3';s.defer=true;s.dataset.fgVoice='1';document.head.appendChild(s)})();
